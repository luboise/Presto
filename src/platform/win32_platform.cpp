...
LRESULT CALLBACK platform_windows_callback(HWND hwnd, UINT uMsg, WPARAM wParam,
                                           LPARAM lParam);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    for (auto i = 0; i < TOTAL_TICKS && running; i++) {
        currentTime = std::chrono::steady_clock::now();

        // Duration of previous poll
        duration = std::chrono::duration_cast<std::chrono::microseconds>(
            currentTime - prevTime);
        std::cout << "Length of previous iteration" << duration << " ≈ "
                  << (currentTime - prevTime) / 1ms << "ms"
                  << std::endl;  // using milliseconds and seconds accordingly
...
HWND setupWindow(HINSTANCE hInstance) {
    ...
    if (!RegisterClass(&MAIN_WC)) {
        MessageBoxA(0, "Failed to register main window class.", "Error",
...
void doPoll() {
    ...
    if (GetRawInputBuffer(NULL, &cbSize, sizeof(RAWINPUTHEADER)) != 0) {
        throw std::invalid_argument(
            "NULL input buffer has a size greater than 0. Invalid access.");
    }
    ...
    for (;;) {
        UINT cbSizeT = cbSize;
        UINT nInput =
            GetRawInputBuffer(pRawInput, &cbSizeT, sizeof(RAWINPUTHEADER));
