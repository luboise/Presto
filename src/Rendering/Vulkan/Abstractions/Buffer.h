...
#define PrestoVulkanBufferCreateArgs                                    \
    ...
    struct Buffer : public Abstraction<VkBuffer> {
        ...
        // Overloads
        void* createMapping() {
            return this->createMapping(0, this->_bufferSize);
        };
        ...
        static void copyInto(CommandPool&, const Buffer&, Buffer&,
                             uint32_t size);
        static void copyInto(CommandPool& command_pool, const Buffer& from,
                             Buffer& to) {
            copyInto(command_pool, from, to,
                     std::clamp(from.getSize(), static_cast<uint32_t>(0),
                                to.getSize()));
        };
