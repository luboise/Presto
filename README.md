...
using Presto::KeyCode;

constexpr auto CAMERA_SPEED = 60;
...
void MainCamera::start() {
    ...
    listener.on<Pr::MouseMovedEvent>([this](Pr::MouseMovedEvent& e) {
        camera_->setRotation(e.GetMouseX() * 0.01, -e.GetMouseY() * 0.1, 0);
    });

    listener.on<Pr::KeyEvent>([this](auto& e) -> void {
        switch (e.getKey()) {
            case KeyCode::UP_ARROW: {
                up_ = e.isPressedEvent();
                break;
            }
            case KeyCode::RIGHT_ARROW: {
                right_ = e.isPressedEvent();
                break;
            }
            case KeyCode::LEFT_ARROW: {
                left_ = e.isPressedEvent();
                break;
            }
            case KeyCode::DOWN_ARROW: {
                down_ = e.isPressedEvent();
                break;
            }
            default: {
                return;
...
void MainCamera::update() {
    float delta{static_cast<float>(Pr::Time::deltaSeconds())};

    Pr::vec3 offset{};

    if (up_) {
        offset += Pr::vec3{0, 0, -CAMERA_SPEED};
    }
    if (down_) {
        offset += Pr::vec3{0, 0, CAMERA_SPEED};
    }
    if (left_) {
        offset += Pr::vec3{-CAMERA_SPEED, 0, 0};
    }
    if (right_) {
        offset += Pr::vec3{CAMERA_SPEED, 0, 0};
    }

    camera_->setPosition(camera_->position() + delta * offset);
};
