...
CommandBuffer::CommandBuffer(CommandPool& pool) : Abstraction(), _pool(pool) {
    ...
    if (vkAllocateCommandBuffers(device.handle(), &allocInfo, &handle_) !=
        VK_SUCCESS) {
...
CommandBuffer::~CommandBuffer() {
    ...
    vkFreeCommandBuffers(device, _pool.handle(), 1, &handle_);
}
...
void CommandBuffer::stopRecording() { vkEndCommandBuffer(handle_); }

void CommandBuffer::bind(Pipeline& pipeline) {
    this->_currentBoundPipeline = &pipeline;

    // Bind the command buffer to the pipeline
    vkCmdBindPipeline(handle_, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      pipeline.handle());
}
...
void CommandBuffer::bind(const BufferList& buffers,
    ...
    if (buffers[0]->getType() == BUFFER_TYPE::VERTEX) {
        vkCmdBindVertexBuffers(handle_, 0, buffers.size(), handles.data(),
                               offsets.data());
    } else if (buffers[0]->getType() == BUFFER_TYPE::INDEX) {
        for (int i = 0; i < buffers.size(); i++) {
            vkCmdBindIndexBuffer(handle_, handles[i], offsets[i],
...
void CommandBuffer::beginPass(const RenderPass& pass,
    ...
    vkCmdBeginRenderPass(handle_, &begin_info, VK_SUBPASS_CONTENTS_INLINE);
