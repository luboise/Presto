...
void DrawManager::submitCommands() {
    ...
    submitInfo.pSignalSemaphores = &_drawContext.sync_set->render_is_finished;
    ...
    presentInfo.pWaitSemaphores = &_drawContext.sync_set->render_is_finished;
    ...
    VkResult res = vkQueuePresentKHR(device->getPresentQueue(), &presentInfo);
...
void DrawManager::createFramebuffers(const Swapchain& swapchain,
    ...
    for (size_t i = 0; i < imageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(swapchain.getDevice()->handle(), &createInfo,
