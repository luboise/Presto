...
namespace Presto {
    ...
    void OpenGLShader::linkShaderProgram() {
        ...
        PR_CORE_ASSERT(
            OpenGLUtils::ShaderProgramLinkedCorrectly(shaderProgram_),
        ...
        std::vector<shader_property_key_t> property_keys{
            "view", "projection", "transform", "sampler1"};

        for (const auto& key : property_keys) {
            propertyMap_[key] =
                glGetUniformLocation(shaderProgram_, key.data());
        }
