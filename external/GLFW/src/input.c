...
static GLFWbool parseMapping(_GLFWmapping* mapping, const char* string) {
    ...
    getLength = strcspn(c, ",");
    if (getLength != 32 || c[getLength] != ',') {
        _glfwInputError(GLFW_INVALID_VALUE, NULL);
        return GLFW_FALSE;
    }

    memcpy(mapping->guid, c, getLength);
    c += getLength + 1;

    getLength = strcspn(c, ",");
    if (getLength >= sizeof(mapping->name) || c[getLength] != ',') {
        _glfwInputError(GLFW_INVALID_VALUE, NULL);
        return GLFW_FALSE;
    }

    memcpy(mapping->name, c, getLength);
    c += getLength + 1;
    ...
    while (*c) {
        ...
        for (i = 0; i < sizeof(fields) / sizeof(fields[0]); i++) {
            getLength = strlen(fields[i].name);
            if (strncmp(c, fields[i].name, getLength) != 0 ||
                c[getLength] != ':')
                continue;
            ...
            if (fields[i].element) {
                ...
                if (strncmp(c, name, getLength) != 0) return GLFW_FALSE;
            }
...
void _glfwInputJoystickHat(_GLFWjoystick* js, int hat, char value) {
    ...
    // Valid hat values only use the least significant nibble and have at most
    // two bits set, which can be considered adjacent plus an arbitrary rotation
    // within the nibble
    assert((value & 0xf0) == 0);
    ...
    switch (mode) {
        case GLFW_CURSOR: {
            if (value != GLFW_CURSOR_NORMAL && value != GLFW_CURSOR_HIDDEN &&
                ...
                _glfwInputError(GLFW_INVALID_ENUM, "Invalid cursor mode 0x%08X",
                                value);
                return;
            }
...
GLFWAPI void glfwSetCursorPos(GLFWwindow* handle, double xpos, double ypos) {
    ...
    if (xpos != xpos || xpos < -DBL_MAX || xpos > DBL_MAX || ypos != ypos ||
        ...
        _glfwInputError(GLFW_INVALID_VALUE, "Invalid cursor position %f %f",
                        xpos, ypos);
        return;
    }
...
GLFWAPI GLFWcursor* glfwCreateCursor(const GLFWimage* image, int xhot,
    ...
    if (image->width <= 0 || image->height <= 0) {
        _glfwInputError(GLFW_INVALID_VALUE,
                        "Invalid image dimensions for cursor");
        return NULL;
    }
...
GLFWAPI GLFWcursor* glfwCreateStandardCursor(int shape) {
    ...
    if (shape != GLFW_ARROW_CURSOR && shape != GLFW_IBEAM_CURSOR &&
        ...
        _glfwInputError(GLFW_INVALID_ENUM, "Invalid standard cursor 0x%08X",
                        shape);
        return NULL;
    }
...
GLFWAPI int glfwUpdateGamepadMappings(const char* string) {
    ...
    while (*c) {
        if ((*c >= '0' && *c <= '9') || (*c >= 'a' && *c <= 'f') ||
            ...
            const size_t getLength = strcspn(c, "\r\n");
            if (getLength < sizeof(line)) {
                _GLFWmapping mapping = {{0}};

                memcpy(line, c, getLength);
                line[getLength] = '\0';
...
GLFWAPI int glfwGetGamepadState(int jid, GLFWgamepadstate* state) {
    ...
    for (i = 0; i <= GLFW_GAMEPAD_BUTTON_LAST; i++) {
        ...
        if (e->type == _GLFW_JOYSTICK_AXIS) {
            const float value =
                js->axes[e->index] * e->axisScale + e->axisOffset;
