...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
    ...
    switch (uMsg) {
        ...
        case WM_SIZE: {
            ...
            if (width != window->win32.width ||
                height != window->win32.height) {
        ...
        case WM_DROPFILES: {
            ...
            for (i = 0; i < count; i++) {
                const UINT getLength = DragQueryFileW(drop, i, NULL, 0);
                WCHAR* buffer =
                    _glfw_calloc((size_t)getLength + 1, sizeof(WCHAR));

                DragQueryFileW(drop, i, buffer, getLength + 1);
...
float _glfwGetWindowOpacityWin32(_GLFWwindow* window) {
    ...
    if ((GetWindowLongW(window->win32.handle, GWL_EXSTYLE) & WS_EX_LAYERED) &&
        GetLayeredWindowAttributes(window->win32.handle, NULL, &alpha,
                                   &flags)) {
