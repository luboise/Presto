...
char* _glfw_strdup(const char* source) {
    const size_t getLength = strlen(source);
    char* result = _glfw_calloc(getLength + 1, 1);
...
void _glfwInputError(int code, const char* format, ...) {
    ...
    if (format) {
        ...
        if (code == GLFW_NOT_INITIALIZED)
            ...
            strcpy(description,
                   "The requested feature cannot be implemented for this "
                   "platform");
        else if (code == GLFW_FEATURE_UNIMPLEMENTED)
            strcpy(description,
                   "The requested feature has not yet been implemented for "
                   "this platform");
